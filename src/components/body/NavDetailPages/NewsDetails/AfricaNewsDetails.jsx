...
const AfricaNewsDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchNewsList = async () => {
      try {
        const newsData = await createClient.fetch(`*[_type == "africaNews" && !(slug.current == "${slug}")]{
          title,
          slug,
          image{
            asset->{
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10  flex flex-col justify-between ">
        ...
        <Link to='/timely-news/' className='text-[#84cc39] hover:underline text-lg  '>Back to News Page</Link>
      </div>
...
export default AfricaNewsDetails;
