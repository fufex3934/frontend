...
function NationalNews() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await createClient.fetch(`*[_type == "nationalNews"]| order(time desc) {
          title,
          slug,
          image{
            asset->{
    ...
    fetchData();
      ...
      const trail = useTrail(nationalNews.length, {
        opacity: 1,
        transform: 'scale(1)',
  ...
  {!loading && (
                ...
                Nationals Most Recent News
                </h2>
          )}
          {
            loading ?(
              <div role="status" className='absolute left-[600px] top-24 transition-all duration-700 '>
              <svg aria-hidden="true" class="w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-[#329898]" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {nationalNews.length > 0 &&
        trail.map((props, index) => (
          <animated.div key={nationalNews[index].slug.current} style={props}>
            <Link to={`/national-news/${nationalNews[index].slug.current}`}>
              <div className="bg-white rounded-lg shadow-xl overflow-hidden hover:shadow-2xl transition duration-300">
                <div className="relative overflow-hidden">
                  <img
                    src={nationalNews[index].image.asset.url}
                    alt={nationalNews[index].title}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black to-transparent">
                    <p className="text-white text-sm flex items-center justify-center">
                      ...
                      {formatDistanceToNow(new Date(nationalNews[index].time), { addSuffix: true })}
                    </p>
                  </div>
                </div>
                <div className="p-6 ">
                  ...
                  <h3 className="text-xl font-bold  text-[#141414] hover:text-orange-700 hover:underline">
                    {`${nationalNews[index].title.substring(0,100)}...`}
                  </h3>
                  </div>
                  <div className="h-24 overflow-hidden">
                    <p className="text-[#141414] mb-2">
                      {`${getDescriptionSubstring(nationalNews[index].description, 200)}...`}
                    </p>
                  </div>
                  <div className='flex items-center justify-between '>
                  <button>
                  <Link to={`/national-news/${nationalNews[index].slug.current}`} className="relative inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-50 group">
