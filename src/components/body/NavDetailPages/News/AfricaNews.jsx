...
function AfricaNews() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await createClient.fetch(`*[_type == "africaNews"]| order(time desc) {
          title,
          slug,
          image{
            asset->{
    ...
    fetchData();
      ...
      const trail = useTrail(africaNews.length, {
        opacity: 1,
        transform: 'scale(1)',
  ...
  <div className=" absolute max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 top-48 md:top-28">
      ...
      {africaNews.length > 0 &&
        trail.map((props, index) => (
          <animated.div key={africaNews[index].slug.current} style={props}>
            <Link to={`/africa-news/${africaNews[index].slug.current}`}>
              <div className="bg-white rounded-lg shadow-xl overflow-hidden hover:shadow-2xl transition duration-300">
                <div className="relative overflow-hidden">
                  <img
                    src={africaNews[index].image.asset.url}
                    alt={africaNews[index].title}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black to-transparent">
                    <p className="text-white text-sm flex items-center justify-center">
                      ...
                      {formatDistanceToNow(new Date(africaNews[index].time), { addSuffix: true })}
                    </p>
                  </div>
                </div>
                <div className="p-6 ">
                  ...
                  <h3 className="text-xl font-bold  text-[#141414] hover:text-orange-700 hover:underline">
                    {`${africaNews[index].title.substring(0,100)}...`}
                  </h3>
                  </div>
                  <div className="h-24 overflow-hidden">
                    <p className="text-[#141414] mb-2">
                      {`${getDescriptionSubstring(africaNews[index].description, 200)}...`}
                    </p>
                  </div>
                  <div className='flex items-center justify-between '>
                  <button>
                  <Link to={`/africa-news/${africaNews[index].slug.current}`} className="relative inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-50 group">
