...
function News() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await createClient.fetch(`*[_type == "home"]| order(time desc) {
          title,
          slug,
          image{
            asset->{
    ...
    fetchData();
      ...
      const trail = useTrail(homeNews.length, {
        opacity: 1,
        transform: 'scale(1)',
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 ">
      {homeNews.length > 0 &&
        trail.map((props, index) => (
          <animated.div key={homeNews[index].slug.current} style={props}>
            <Link to={`/news/${homeNews[index].slug.current}`}>
              <div className="bg-white rounded-lg shadow-xl overflow-hidden hover:shadow-2xl transition duration-300">
                <div className="relative overflow-hidden">
                  <img
                    src={homeNews[index].image.asset.url}
                    alt={homeNews[index].title}
                    className="w-full h-48 object-cover"
                  />
                  <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black to-transparent">
                    <p className="text-white text-sm flex items-center justify-center">
                      <BiTimeFive className='mr-1'/>
                      {formatDistanceToNow(new Date(homeNews[index].time), { addSuffix: true })}
                    </p>
                  </div>
                </div>
                <div className="p-6 ">
                  ...
                  <h3 className="text-xl font-bold  text-[#141414] hover:text-orange-700 hover:underline">
                    {`${homeNews[index].title.substring(0,100)}...`}
                  </h3>
                  </div>
                  <div className="h-24 overflow-hidden">
                    <p className="text-[#141414] mb-2">
                      {`${getDescriptionSubstring(homeNews[index].description, 200)}...`}
                    </p>
                  </div>
                  <div className='flex items-center justify-between '>
                  <p className="text-sm text-[#141414] font-semibold italic">{homeNews[index].category}</p>
                  <button>
                  <Link to={`/news/${homeNews[index].slug.current}`} className="relative inline-flex items-center justify-start py-3 pl-4 pr-12 overflow-hidden font-semibold text-indigo-600 transition-all duration-150 ease-in-out rounded hover:pl-10 hover:pr-6 bg-gray-50 group">
